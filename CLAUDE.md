...
A separate CLI orders the whole level **sequence** into a difficulty curve: `scripts/order-by-difficulty.ts <levelsDir> <orderFile> [--write]` scores each level from the analyzers (`D = 0.5·tightness + 0.3·load + 0.2·length` — tightness = A\* optimal-moves / move-limit, load = categories − slots, length = optimal moves, each min-max normalized; greedy-softlock levels forced to the top band), then rank-matches the ranking onto a **sawtooth curve** (each 5-level cycle ramps easy→hard, no macro drift) and rewrites the Unity **level-order array** — `…/SoliJong/unity/soli-jong/Assets/Game/LevelOrder/default.json`, a JSON list of `levelId`s loaded at runtime by `LevelOrderProvider` (`MainLevelTrack`; array position = play order). The rewrite is lossless (permutes the existing ids after a pure-permutation gate, preserves the one-id-per-line format) and recomputes A\* (~1–2 min for 50 levels). Edit `targetOf()` to change the curve shape; `DEBUG_RANK=1` dumps the per-level ranking. Note the level-order file lives in the shipping Unity project, **not** `src/levels`.
